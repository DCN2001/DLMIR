...
class DAESkipConnections(nn.Module):
    def __init__(self):
        ...
        self.encoder_conv5 = nn.Sequential(             #Addition
            nn.Conv2d(128, 256, 3, 1, 1),
            nn.BatchNorm2d(256),
            nn.ReLU()
        )
        self.decoder_conv0 = nn.Sequential(             #Addition
    ...
    def forward(self, x):
        ...
        encoder_out5 = self.encoder_conv5(encoder_out4)
        decoder_out = self.decoder_conv0(encoder_out5)
        decoder_out = self.decoder_conv1(torch.cat((decoder_out, encoder_out4), dim=1))
